...
// ── CONFIG ──
// Same-origin API — worker serves both static files and /api/* from one URL.
// If you deploy the worker separately, replace with the full worker URL, e.g.:
//   const API_BASE = 'https://vaultdl.yourname.workers.dev/api';
const API_BASE = '/api';
...
async function fetchVideo() {
  ...
  try {
    ...
    const msg = err?.message?.includes('fetch')
      ? 'Could not reach the server. Make sure the Worker is deployed and try again.'
